...
export const authUser = (fields, role, mode) => async (dispatch) => {
    ...
    try {
        const result = await axios.post(`${url}/${role}${mode}`, fields, {
...
export const addStuff = (address, fields) => async (dispatch) => {
    ...
    try {
        const result = await axios.post(`${url}/${address}`, fields, {
            headers: { 'Content-Type': 'application/json' },
        });
...
export const updateCustomer = (fields, id) => async (dispatch) => {
    ...
    try {
        await axios.put(`${url}/CustomerUpdate/${id}`, newFields, {
            headers: { 'Content-Type': 'application/json' },
        });
...
export const getProductsbySeller = (id) => async (dispatch) => {
    ...
    try {
        const result = await axios.get(`${url}/getSellerProducts/${id}`);
        if (result.data.message) {
...
export const getProductDetails = (id) => async (dispatch) => {
    ...
    try {
        const result = await axios.get(`${url}/getProductDetail/${id}`);
        if (result.data.message) {
...
export const getSearchedProducts = (address, key) => async (dispatch) => {
    ...
    try {
        const result = await axios.get(`${url}/${address}/${key}`);
        if (result.data.message) {
